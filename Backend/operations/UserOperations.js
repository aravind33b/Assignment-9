...
var emailVal = /^\w+([\.-]?\w+)*@(northeastern.edu|gmail.com)$/;
var nameVal = /^([\w\.\,\ ]+){2,}$/;
...
exports.authenticateUser = async (req, res) => {
  try {
    ...
    const userDetails = await service.getUser(req.body.email);
    if(!userDetails){
      res.status(401).json({ data: { isUserLoggedIn: false, message: "User not found"}});
      return;
    }
    const isValidUser=  await comparePassword(password, userDetails.password);

    if(isValidUser){
      const userName = userDetails.name;
      res.status(200).json({ data: { userName , isUserLoggedIn: true, message: "User authentication successful"}});
    } else {
      res.status(401).json({ data: { isUserLoggedIn: false, message: "Invalid credentials"}});
      return;
    }
  } catch (err) {
    res.status(500).json({ error: err.message });
